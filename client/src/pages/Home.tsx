    ...
    <div className="min-h-screen bg-white text-gray-900">
      ...
      <section className="relative pt-20 pb-24 px-6 text-center overflow-hidden">
        ...
        <div className="relative max-w-3xl mx-auto">
          ...
          <h1 className="font-black tracking-tight leading-tight mb-3">
            <span className="block text-5xl md:text-6xl lg:text-7xl" style={{ color: NAVY }}>
              AID ANALYTICS
            </span>
            <span className="block text-3xl md:text-4xl lg:text-5xl text-orange-500 mt-1">
              for Volleyball
            </span>
          </h1>

          <p className="text-gray-500 text-lg md:text-xl leading-relaxed mt-7 mb-10 max-w-xl mx-auto">
            全プレーをリアルタイムで記録・分析。<br />
            Gemini AIが戦況を読み解き、勝利への洞察を届けます。
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center mb-7">
            <Link href="/setup">
              <Button
                size="lg"
                className="text-white font-bold px-8 py-6 text-base rounded-xl shadow-lg transition-all hover:opacity-90"
                style={{ backgroundColor: NAVY, boxShadow: `0 8px 24px ${NAVY}30` }}
              >
      ...
      <section className="border-y border-gray-100 bg-white py-8 px-6">
        <div className="max-w-4xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
          {[
            { value: "3秒", label: "平均記録時間 / プレー", color: "text-orange-500" },
            { value: "9種", label: "分析ビューの数", color: "text-orange-500" },
            { value: "AI", label: "Gemini 2.5 Flash 搭載", color: "text-orange-500" },
            { value: "∞", label: "同時閲覧端末数", color: "text-orange-500" },
          ].map((s) => (
            <div key={s.label}>
              <div className={`text-4xl font-black ${s.color} leading-none`}>{s.value}</div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            {[
              ...
              },
              {
                ...
                title: "点数推移グラフ",
                desc: "ラリーごとのスコア変化をリアルタイムでライングラフ表示。ターニングポイントを一目で把握。",
              },
              {
                icon: Sparkles,
                iconBg: "bg-amber-100",
                ...
                title: "AI 戦術提案",
                desc: "Gemini 2.5 Flashが試合データを解析し、タイムアウト・セット間に実行可能な戦術提案を生成。",
              },
              {
                icon: BarChart2,
                ...
                title: "9種類の分析ビュー",
                desc: "概要・選手別・チーム比較・ヒートマップ・ポジション・時系列など多角的な分析タブ。",
              },
              {
                icon: Mic,
                iconBg: "bg-green-100",
                ...
                title: "音声入力対応",
                desc: "「7番アタック成功」などの音声実況からプレーを自動抽出。ハンズフリーで高速記録。",
              },
              {
                ...
                title: "Excel エクスポート",
                desc: "試合データをExcel形式でエクスポート。選手別・セット別の詳細データを外部で活用可能。",
              },
            ...
            {[
              {
                step: "01",
                title: "チームと試合を登録",
                desc: "選手情報を登録して試合を作成。相手チームもその場で選択・登録できます。試合コードが自動生成されます。",
                checks: ["選手・背番号の登録", "相手チームの選択", "試合コードの自動発行"],
              },
              {
                step: "02",
                title: "タップでデータ入力",
                desc: "データ入力画面で選手を選びプレーをタップ記録。複数端末から同時入力も可能。",
                checks: ["1プレー3〜4タップ", "複数端末の同時入力", "音声入力にも対応"],
              },
              {
                step: "03",
                title: "コーチ画面で分析",
                desc: "試合コードを共有するだけでコーチ・スタッフがリアルタイムで統計を閲覧できます。",
                checks: ["スコア推移グラフ", "選手別パフォーマンス", "AIによる戦術提案"],
              },
            ].map(({ step, title, desc, checks }) => (
              <div
                key={step}
                className="rounded-2xl border border-gray-100 p-7 relative overflow-hidden"
              >
                <div
      ...
      {/* ═════════════════════════════��════════════
          5. SPOTLIGHT — AI分析
      ══════════════════════════════════════════ */}
      <section className="py-20 px-6" style={{ backgroundColor: `${NAVY}08` }}>
        <div className="max-w-5xl mx-auto">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <h2 className="text-3xl md:text-4xl font-black tracking-tight mb-6" style={{ color: NAVY }}>
                タイムアウトの<br />
                <span className="text-orange-500">15秒</span>を、<br />
                最大限に活かす
              </h2>
              <p className="text-gray-600 leading-relaxed mb-6">
                Gemini 2.5 Flashがリアルタイムで試合データを解析。
                相手チームの攻撃パターン・弱点・調子のよい選手など、
                コーチが必要な情報を即座に日本語でまとめます。
              </p>
            ...
            {/* visual panel */}
            <div className="rounded-2xl border bg-white p-6 shadow-lg" style={{ borderColor: `${NAVY}15` }}>
              ...
              {[
                {
                  title: "相手の攻撃傾向",
                  body: "#7 はクロス方向への攻撃が全体の68%。レシーブの右寄りポジション調整を推奨。",
                  accent: "orange",
                },
                {
                  title: "サーブの狙い目",
                  body: "#3 のレシーブ成功率が42%と低下中。積極的にサーブを集めることで崩せる可能性が高い。",
                  accent: "navy",
                },
                {
                  title: "選手交代の検討",
                  body: "#11 の最近5プレーでエラーが3回。#15 への交代でレシーブ安定が期待できます。",
                  accent: "navy",
                },
              ].map(({ title, body, accent }) => (
                <div key={title} className="mb-4 last:mb-0 p-4 rounded-xl bg-gray-50 border border-gray-100">
                  <div
                    className="text-xs font-bold mb-1.5"
                    style={{ color: accent === "orange" ? "#f97316" : NAVY }}
                  >
                    {title}
                  </div>
          ...
          <div className="flex flex-col md:flex-row items-center gap-10">
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-sm font-bold text-gray-400 tracking-widest uppercase">Multi-Device</span>
              </div>
              <h2 className="text-2xl md:text-3xl font-black mb-4" style={{ color: NAVY }}>
                役割分担で、<br />もっとスムーズに
              </h2>
              <p className="text-gray-500 leading-relaxed mb-4">
                試合コードを共有するだ���で、記録・閲覧・分析を複数端末から同時に行えます。
                マネージャーが入力しながらコーチがリアルタイムで分析できます。
              </p>
            </div>
            <div className="flex-1 grid grid-cols-2 gap-3">
              {[
                { role: "データ入力", device: "スマホ / タブレット", icon: Zap, who: "マネージャー・スタッフ" },
                { role: "コーチ閲覧", device: "タブレット / PC", icon: BarChart2, who: "コーチ・監督" },
                { role: "AI分析", device: "任意の端末", icon: Sparkles, who: "チーム全員で共有" },
                { role: "試合記録", device: "クラウド保存", icon: ShieldCheck, who: "いつでも振り返り可能" },
              ].map(({ role, device, icon: Icon, who }) => (
                <div
                  key={role}
                ...
                >
                  ...
                  <div className="text-xs text-gray-500 mt-0.5">{device}</div>
                  <div className="text-xs mt-2 font-medium" style={{ color: NAVY }}>{who}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="py-16 px-6 text-white" style={{ backgroundColor: NAVY }}>
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8">
          <div>
            ...
            <h2 className="text-2xl md:text-3xl font-black mb-1">今すぐ始める</h2>
            <p className="text-white/50 text-sm">チーム登録から分析まですべて無料</p>
          </div>
          <div className="flex flex-wrap gap-3">
            <Link href="/setup">
              <Button
                size="lg"
                className="bg-orange-500 hover:bg-orange-400 text-white font-bold rounded-xl px-7 py-5 transition-colors"
              >
                <Trophy className="w-4 h-4 mr-2" />
                試合を開始する
              </Button>
            ...
            <Link href="/teams">
              <Button variant="outline" className="border-white/20 text-white/80 hover:bg-white/10 bg-transparent rounded-xl px-6 py-5">
